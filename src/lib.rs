...
impl NoiseConfig {
    ...
    pub fn new(octaves: i32, x_frequency: f32, y_frequency: f32, amplitude: f32, 
    lacunarity: f32, gain: f32, range: (f32, f32), seed: u64) -> Self {
        
        // Define Permutation.
        let permutation: [u16; 512] = set_seed(seed);
        // Create a new noise config, prepare to return it. 
        let result = NoiseConfig {
            octaves, x_frequency, y_frequency, amplitude,
            lacunarity, gain, range, seed, permutation,
        };
        return result;
    }
    ...
    /// generates raw values, and converts those values to your specified range without FBM. 
    /// returns a simplex noise value WITHOUT Fractal Brownian Motion. It is converted into a specified range. 
    pub fn generate_raw_range(&self, x: f32, y: f32) -> f32 {
        return convert_range(MAX, MIN, self.range.0, self.range.1, generate(x, y, &self.permutation));
    }
    ...
    pub fn output(&self, size: i32, vector: &Vec<char>) {
        ...
        for v in 0..values.len() {
            for c in 0..vector.len() {
                if (values[v] > (min + (dif * c as f32))) && ((min + (dif * (c as f32 + 1.0))) > values[v]) { 
                    graph.push(vector[c]);
                    break;
                }
            }
    ...
    pub fn output_1d(&self, length: i32, height: i32) {
        ...
        println!("\n Max Value: {}, Min Value: {}, Difference: {}",max, min, dif);
